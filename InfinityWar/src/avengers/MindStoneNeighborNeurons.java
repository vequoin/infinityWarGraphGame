...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        HashSet<String> outVertices = new HashSet<>(); 

        int numEdges = StdIn.readInt();

        for(int i = 0; i < numEdges;i++){
            String edgeO = StdIn.readString();
            String edgeE = StdIn.readString();
            outVertices.add(edgeO);
            AdjList.put(edgeO, edgeE);
        }
        String mindstone = null;

        for(int i = 0; i < vertices.length;i++){
            if(!outVertices.contains(vertices[i])){
                mindstone = vertices[i];
            }
        }
        
        for(int i = 0; i < AdjList.size();i++){
            if(AdjList.get(vertices[i]).equals(mindstone)){
                StdOut.println(vertices[i]);
            }
        }
